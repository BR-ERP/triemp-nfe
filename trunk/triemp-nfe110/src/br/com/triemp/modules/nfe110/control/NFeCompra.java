...
public class NFeCompra extends NFe {
	...
	private void carregaItCompra() {
		...
		String sql = "SELECT PRO.CODEANPROD, PRO.DESCPROD, PRO.CODUNID, PRO.TIPOPROD, NATOP.DESCNAT, CLF.CODFISC, CLF.CODNCM, CLF.CODSERV, " 
				+ "CLFIT.CODITFISC, CLFIT.CODTRATTRIB, CLFIT.ORIGFISC, CLFIT.MODBCICMS, CLFIT.MODBCICMSST,  CLFIT.REDFISC, CLFIT.REDBASEST, CLFIT.CODSITTRIBIPI, CLFIT.TPCALCIPI, CLFIT.VLRIPIUNIDTRIB, CLFIT.CODSITTRIBPIS, CLFIT.ALIQPISFISC, CLFIT.VLRPISUNIDTRIB, CLFIT.CODSITTRIBCOF, CLFIT.ALIQCOFINSFISC, CLFIT.VLRCOFUNIDTRIB, "
				...
				+ "WHERE IT.CODEMP=? AND IT.CODFILIAL=? AND IT.CODCOMPRA=?";
		try {
			ps = conSys.prepareStatement(sql);
			ps.setInt(1, (Integer) key.get(FreedomNFEKey.CODEMP));
			ps.setInt(2, (Integer) key.get(FreedomNFEKey.CODFILIAL));
	...
	protected void carregaInfTransporte() {
		PreparedStatement ps;
		ResultSet rs;
		String sql = "SELECT tra.RAZTRAN, tra.CNPJTRAN, tra.CPFTRAN, tra.INSCTRAN, tra.ENDTRAN, tra.CIDTRAN, tra.UFTRAN, tra.CODMUNIC, "
			+ "uf.CODUF, fre.TIPOFRETECP, fre.CONHECFRETECP, fre.PLACAFRETECP, fre.UFFRETECP, fre.QTDFRETECP, fre.PESOBRUTCP, fre.PESOLIQCP, fre.ESPFRETECP, fre.MARCAFRETECP, cfre.VLRFRETE, cfre.ALIQICMSFRETE, cfre.VLRICMSFRETE, cfre.VLRBASEICMSFRETE, cfre.CODNAT "
		...
		try {
			...
			if (rs.next()) {
				...
				transp.setModFrete(getInteger("9", 1, true)); // 9- Sem frete. (V2.0)
			}
