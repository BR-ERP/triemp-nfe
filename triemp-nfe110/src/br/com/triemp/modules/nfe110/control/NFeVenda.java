...
public class NFeVenda extends NFe {
		...
		try {
			...
			if (rs.next()) {
				...
				if(getString(rs.getString("SIMPLESFILIAL")).equals("S")){
					setInfAdFisco(NFeUtil.geraMensagens(msgSimples, 
							getDouble(rs.getString("VLRICMSSIMPLES"), 15, 2, true), 
							getDouble(rs.getString("PERCICMSSIMPLES"), 15, 2, true)));
				}
	...
	private void carregaDestinatarioNF(String codCli) {
		...
		try {
			...
			if (rs.next()) {
				...
				if( (rs.getString("ENDCLI") == null || rs.getString("ENDCLI").length() < 2) 
						|| (rs.getString("NUMCLI") == null || rs.getString("NUMCLI").length() < 1) 
						|| (rs.getString("BAIRCLI") == null || rs.getString("BAIRCLI").length() < 1) 
						|| (cMun == null || cMun.length() < 7) ){
					String msg = "Cadastro de cliente incompleto!";
					String msgCorr = "Os seguintes dados do cliente são obrigatórios:\nRazão, CPF/CNPJ, IE, Logradouro, Número, Bairro, Cidade, Estado";
				...
				if(rs.getString("EMAIL") != null){
					emailNfe = getString(rs.getString("EMAIL"));
				}
	...
	private void carregaItVenda() {
		...
		String sql = "SELECT pro.CODBARPROD, pro.DESCPROD, pro.CODUNID, pro.TIPOPROD, natop.DESCNAT, clf.CODFISC, clf.CODNCM, clf.CODSERV, "
				+ "clfit.CODITFISC, clfit.ORIGFISC, clfit.MODBCICMS, clfit.MODBCICMSST,  clfit.REDFISC, clfit.REDBASEST, clfit.CODSITTRIBIPI, clfit.TPCALCIPI, clfit.VLRIPIUNIDTRIB, clfit.CODSITTRIBPIS, clfit.ALIQPISFISC, clfit.VLRPISUNIDTRIB, clfit.CODSITTRIBCOF, clfit.ALIQCOFINSFISC, clfit.VLRCOFUNIDTRIB, " 
				...
				+ "FROM VDITVENDA it INNER JOIN EQPRODUTO pro ON (pro.CODPROD = it.CODPROD) LEFT JOIN LFNATOPER natop ON (natop.CODNAT = it.CODNAT) LEFT JOIN LFCLFISCAL clf ON (clf.CODFISC = pro.CODFISC) LEFT JOIN LFITCLFISCAL clfit ON (clf.CODFISC = clfit.CODFISC AND clfit.CODITFISC = it.CODITFISC) "
				+ "WHERE it.CODEMP=? AND it.CODFILIAL=? AND it.CODVENDA=? AND it.TIPOVENDA='V' AND it.CANCITVENDA IS NULL";
		try {
			ps = conSys.prepareStatement(sql);
			ps.setInt(1, (Integer) key.get(FreedomNFEKey.CODEMP));
			ps.setInt(2, (Integer) key.get(FreedomNFEKey.CODFILIAL));
			...
			while (rs.next()) {
				...
				// Verificar se o valor do seguro é adicionado no VDITVENDA.VLRADICITVENDA
				...
				if (rs.getString("TIPOPROD").trim().equals("S")) {
					...
					if(cst != null){
						...
						if (cst.equals("00")) {
							...
							tipoIcms.setVBCST("0.00");
							tipoIcms.setVICMSST("0.00");
	...
	protected void carregaInfTransporte() {
		PreparedStatement ps;
		ResultSet rs;
		String sql = "SELECT tra.RAZTRAN, tra.CNPJTRAN, tra.CPFTRAN, tra.INSCTRAN, tra.ENDTRAN, tra.CIDTRAN, tra.UFTRAN, tra.CODMUNIC, uf.CODUF, "
	...
	private void geraLogNFe(){
		...
		try {
			...
			ps.setInt(3, (Integer) key.get(FreedomNFEKey.CODVENDA));
			ps.setString(4, "V");
			ps.setString(5, infNFe.getId().replace("NFe", ""));
			ps.setString(6, "");
			ps.setString(7, infNFe.getVersao());
			ps.setString(8, "G");
			ps.setString(9, "ARQUIVO XML DA NOTA FISCAL ELETRÔNICA DE VENDA GERADO COM SUCESSO NO DIRETÓRIO: "+path);
			ps.execute();
